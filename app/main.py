...
async def home(request: Request, store: Store = Depends(store_dep)):
    ...
    scored = [
        {**c, "uti": compute_uti_score(c), "policy": recommend_checkout_policy(compute_uti_score(c))}
        for c in customers
    ]
...
async def dispute_submit(
    ...
    payload = DisputeRequest(
        ...
        photos_show_damage=photos_show_damage == "yes",
...
async def _run_dispute(body: DisputeRequest, store: Store) -> dict[str, Any]:
    ...
    if body.empty_box_reported or not body.package_seal_intact:
        ...
        liable = "Meesho / Delivery partner (tamper or empty box)"
        customer_delta = -40
        rider_delta = -35
        mfr_delta = 0
    elif body.photos_show_damage:
    ...
    gemini = await analyze_dispute_with_gemini(
        {
            ...
            "photos_show_damage": body.photos_show_damage,
    ...
    # Apply score deltas to in-memory profiles for demo
